...
<TABLE bgcolor="#FFFFFF" cellSpacing=0 cellPadding=0 width=962 height="300" align=center border=0><!--DWLayoutTable-->
  ...
  <TR>
    ...
    <script language="JavaScript">
		function change(str) {
		window.location = 'listadmin.php?s='+str;
}
</script>
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <select id="menuid" name="menuid" onchange="change(this.value)">
                        		<?php
			include("dbconnect.php");
			...
			{
				echo("<option ");
				if ($_REQUEST['s']==$row0['menuid']) echo("selected='selected'");
		...
		?>
          ...
          <td width="161" align="right" valign="middle"><div align="right"><img src="pic/add.jpg" align="middle">&nbsp;&nbsp;<a href="listadminaddnew.php" class="nav">Thêm mới</a> </div></td>
          <td width="53">&nbsp;</td>
        </tr>
        <tr>
          <td height="29">&nbsp;</td>
            ...
            <TBODY>
              <TR>
                <TD width=181 height=27 align=middle vAlign=center 
                      background=pic/bgbt.gif class="table_title">Thuộc nhóm/menu </TD>
                    <TD width=404 align=middle vAlign=center 
                      background=pic/bgbt.gif class="table_title">Tiêu đề liên kết </TD>
              ...
              <?php
					...
					if ($mode=='1')
					{
						$str1="delete from list where listid=". $id;
						$result=mysql_query($str1) or die(mysql_error());
						//mysql_free_result($result);
					}
					
					//Cap nhat muc trong danh sach lien ket duoc chon
					if ($mode=='2')
					{
						$listid=intval(mysql_real_escape_string($_REQUEST['id']));
						$menuid=intval(mysql_real_escape_string($_REQUEST['menuid']));
						$listtitle=mysql_real_escape_string($_REQUEST['listtitle']);
						$listcontent=$_REQUEST['listcontent'];
						$listorder=intval(mysql_real_escape_string($_REQUEST['listorder']));
						$file=mysql_real_escape_string($_REQUEST['file']);
						
						$str1="update list set menuid=".$menuid.", listtitle='".$listtitle."', listcontent='".$listcontent;
						$str1=$str1."', listorder=".$listorder.", file='".$file."' where listid=". $listid;
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}						
					

					//Them 01 muc moi trong danh sach lien ket
					if ($mode=='3')
					{
						$menuid=intval(mysql_real_escape_string($_REQUEST['menuid']));
						$listtitle=mysql_real_escape_string($_REQUEST['listtitle']);
						...
						$str1="insert into list(menuid,listtitle,listcontent,listorder,file) ";
						$str1=$str1." values (".$menuid.",'".$listtitle."','".$listcontent."',".$listorder.",'".$file."')";
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}					
					
					
					//Hien thi danh sach cac lien ket
					$str1="select list.*,menutitle from list,menu where list.menuid=menu.menuid order by menuid,listorder, listid";
					//$str1="select * from list";
					if($_REQUEST['s']>0)
					$str1="select list.*,menutitle from list,menu where list.menuid='".$_REQUEST['s']."' and menu.menuid='".$_REQUEST['s']."' order by menuid,listorder, listid";
					$result=mysql_query($str1) or
						die(mysql_error());
					$i=0;
					//phan trang
					$c=0;
					RecordsIgnore2($row,$result);	
					while ($row=mysql_fetch_array($result))
					{
						...
						echo("<TD vAlign=left align=middle>".$row['listtitle']."</TD>");
						echo("<TD vAlign=center align=middle><center><a href='listadminupdate.php?id=".$row['listid']."'>Sửa</a>");
						echo("&nbsp;&nbsp;&nbsp;<a href='listadmin.php?mode=1&id=".$row['listid']."'>Xóa</a></center></TD></TR>");
        ...
        <tr>
		  ...
		  <?php
		  	$selectcountsql="select count(*) as sumrecords from list order by listid desc";
			ShowPageBreak2($selectcountsql,"listadmin.php");
		  ?>		  
		  </td>
          <td>&nbsp;</td>
        </tr>

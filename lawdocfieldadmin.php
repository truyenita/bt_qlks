    ...
    <TD valign="top"><table width="295" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="52" height="29"><!-- InstanceBeginEditable name="EditRegion1" --><img src="pic/timetable.jpg"><!-- InstanceEndEditable --></td>
            <td width="243" valign="top" class="lefttitle"><!-- InstanceBeginEditable name="EditRegion2" -->Cập nhật lĩnh vực văn bản <!-- InstanceEndEditable --></td>
          </tr>
      <tr>
        <td height="3" colspan="2" valign="top"><table background="pic/hline.jpg" width="295" border="0" cellpadding="0" cellspacing="0">
          <!--DWLayoutTable-->
        ...
        <tr>
            ...
            <TBODY>
              <TR>
                <TD width=189 height=27 align=middle vAlign=center 
                      background=pic/bgbt.gif class="table_title">Tên lĩnh vực </TD>
              ...
              <?php
					...
					if ($mode=='1')
					{
						$str1="delete from lawdocfield where fieldid=". $id;
					...
					if ($mode=='2')
					{
						$fieldid=mysql_real_escape_string($_REQUEST['id']);
						$fieldname=mysql_real_escape_string($_REQUEST['fieldname']);

						$str1="update lawdocfield set fieldname='".$fieldname."' where fieldid=". $fieldid;
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}						
					...
					if ($mode=='3')
					{
						...
						$str1="insert into lawdocfield(fieldname) values ('".$fieldname."')";
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}					
										
					//Hien thi danh sach dia ban doanh nghiep
					$str1="select * from lawdocfield order by fieldname";
					...
					while ($row=mysql_fetch_array($result))
					{
						...
						echo("<TD vAlign=center height=24>&nbsp;&nbsp;".$row['fieldname']."</TD>");
						echo("<TD vAlign=center align=middle><center><a href='lawdocfieldadmin.php?mode=20&id=".$row['fieldid']."'>Sửa</a>");
						echo("&nbsp;&nbsp;&nbsp;<a href='lawdocfieldadmin.php?mode=1&id=".$row['fieldid']."'>Xóa</a></center></TD></TR>");
					}
					mysql_free_result($result);
				?>
              </TBODY>
          ...
          <td rowspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
           ...
           <?php
		   		///////// Cap nhat
				if ($mode==20)
					$action="lawdocfieldadmin.php?mode=2&id=".$id;
				else
					if ($mode==30)
						$action="lawdocfieldadmin.php?mode=3";
					else	
						$action="lawdocfieldadmin.php";
		   ?>
		   <form name='form' action="<?php echo($action);?>" method="post">
		    <tr>
              <td width="107" height="29" valign="middle"><strong>Tên lĩnh vực </strong></td>
              <td width="261" valign="middle"><label>
                <input name="fieldname" type="text" id="fieldname" size="30" maxlength="200" 
				value="<?php 
				$id=mysql_real_escape_string($_REQUEST['id']);
				if ($id<1) $id=-2;
				echo(getValue('fieldname','lawdocfield','fieldid',$id));?>">
              </label></td>
            </tr>
            <tr>
              <td height="29">&nbsp;</td>
              <td valign="middle">
				...
				onclick="window.open('lawdocfieldadmin.php?mode=30','_self')"></td>
              </tr>
			 </form> 

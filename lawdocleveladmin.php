    ...
    <TD valign="top"><table width="295" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="52" height="29"><!-- InstanceBeginEditable name="EditRegion1" --><!-- InstanceEndEditable --></td>
            <td width="243" valign="top" class="lefttitle"><!-- InstanceBeginEditable name="EditRegion2" -->Cập nhật cấp ban hành văn bản<!-- InstanceEndEditable --></td>
          </tr>
      <tr>
        <td height="3" colspan="2" valign="top"><table background="pic/hline.jpg" width="295" border="0" cellpadding="0" cellspacing="0">
          <!--DWLayoutTable-->
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td valign="top"><TABLE class="deco" height=29 cellSpacing=0 cellPadding=0 
            ...
            <TBODY>
              ...
              <?php
					...
					if ($mode=='1')
					{
						$str1="delete from lawdoclevel where levelid=". $id;
					...
					if ($mode=='2')
					{
						$levelid=mysql_real_escape_string($_REQUEST['id']);
						$levelname=mysql_real_escape_string($_REQUEST['levelname']);

						$str1="update lawdoclevel set levelname='".$levelname."' where levelid=". $levelid;
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}						
					...
					$str1="insert into lawdoclevel(levelname) values('".$levelname
					."')";
						//echo($str1);
						$result=mysql_query($str1) or die(mysql_error());
					}					
					...
					$str1="select * from lawdoclevel order by levelname";
					...
					while ($row=mysql_fetch_array($result))
					{
						...
						echo("<TD vAlign=center height=24>&nbsp;&nbsp;".$row['levelname']."</TD>");
						echo("<TD vAlign=center align=middle><center><a href='lawdocleveladmin.php?mode=20&id=".$row['levelid']."'>Sửa</a>");
						echo("&nbsp;&nbsp;&nbsp;<a href='lawdocleveladmin.php?mode=1&id=".$row['levelid']."'>Xóa</a></center></TD></TR>");
					}
					mysql_free_result($result);
				?>
              </TBODY>
          ...
          <td rowspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
           ...
           <?php
		   		///////// Cap nhat
				if ($mode==20)
					$action="lawdocleveladmin.php?mode=2&id=".$id;
				else
					if ($mode==30)
						$action="lawdocleveladmin.php?mode=3";
					else	
						$action="lawdocleveladmin.php";
		   ?>
		   <form name="form" action="<?php echo($action);?>" method="post">
		    <tr>
              ...
              <td width="261" valign="middle"><label>
                <input name="levelname" type="text" id="levelname" size="30" maxlength="200" 
				value="<?php 
				$id=mysql_real_escape_string($_REQUEST['id']);
				if ($id<1) $id=-2;
				echo(getValue('levelname','lawdoclevel','levelid',$id));?>">
              </label></td>
            </tr>
            <tr>
              <td height="29">&nbsp;</td>
              <td valign="middle">
				...
				onclick="window.open('lawdocleveladmin.php?mode=30','_self')"></td>
              </tr>
			 </form> 
	 <SCRIPT language=JavaScript type=text/JavaScript>
					<!--
 					function Check()
					{
						if(document.form.levelname.value=="")
						{
							alert("Bạn chưa nhập tên cấp ban hành văn bản!");
							document.form.levelname.focus();
							return false;
						}
					}

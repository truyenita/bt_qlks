...
<div class="container">
          ...
          <tr>
            <td width="561" height="186" valign="top">
				...
				///////////////////////////////////Hien thi noi dung trang listdetail
				include("dbconnect.php");
				//echo("Hello!");
				$listid=intval(mysql_real_escape_string($_REQUEST['listid'])) ;
				$str1="select listcontent,file from list where listid=".$listid;
				$result=mysql_query($str1) or
					die(mysql_error());
				$row=mysql_fetch_array($result);
	
				//echo("<div  class='newstitleNOLINK'>".$row['title']."</div>"); 
				//echo("<br>");
				//echo("<br>");
				echo("<div  class='newscontent'>".$row['listcontent']."</div>");
				echo("<BR>");
				if (strlen($row['file'])>0)
					echo("<a class='newstitlelink' href='".$row['file']."'>Download tài liệu đính kèm</a>");

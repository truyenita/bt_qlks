    ...
    <TD valign="top"><table width="295" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="52" height="29"><!-- InstanceBeginEditable name="EditRegion1" -->aa<!-- InstanceEndEditable --></td>
            <td width="243" valign="top" class="lefttitle"><!-- InstanceBeginEditable name="EditRegion2" -->Sửa ảnh<!-- InstanceEndEditable --></td>
          </tr>
      <tr>
        <td height="3" colspan="2" valign="top"><table background="pic/hline.jpg" width="295" border="0" cellpadding="0" cellspacing="0">
          <!--DWLayoutTable-->
  ...
  <TR>
    ...
    <TD colspan="2" valign="top"><!-- InstanceBeginEditable name="EditRegion3" -->
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <!--DWLayoutTable-->
        <tr>
              <td height="458" valign="top">Sửa hình ảnh<br />
    <form id="form1" name="form1" method="post" action="hinhanh.php?editid=<?php echo($_REQUEST["editid"]); ?>" enctype="multipart/form-data">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <?php 
	  //include("../dbconnect.php");
	  $sql_edit_ha = "select * from album2 where `id`='".$_REQUEST["editid"]."'";
	  $result_edit_ha = mysql_query($sql_edit_ha) or die(mysql_error());
	  while($row_edit_ha = mysql_fetch_array($result_edit_ha))
	  {
	  ?>
        <tr>
          <td width="20%">Hình:</td>
          <td width="80%"><input name="duongdananh" type="text" id="duongdananh" value="<?php echo($row_edit_ha["duongdananh"]); ?>">
            <input name="submit" type="button" value="Chọn Image" onClick="BrowseServer();">            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="<?php echo($row_edit_ha["duongdananh"]); ?>" border="0" height="45" width="60" />          </td>
        </tr>
        <tr>
          <td>Ghi chú:</td>
          <td>
            <input type="text" name="ghichu" id="ghichu" value="<?php echo($row_edit_ha["ghichu"]); ?>"/>
          </td>
        </tr>
		<tr>
          <td>Chọn Album:</td>
          <td>
            <select name="idalbum1" id="idalbum1">
			  <?php
				$sql_ha = "select * from album1"; 
				$result_ha = mysql_query($sql_ha) or die (mysql_error());
				while($row_ha=mysql_fetch_array($result_ha))
				{
				?>
				<option value="<?php echo($row_ha["id"]); ?>" <?php echo($row_ha["id"]==$row_edit_ha["idalbum1"]?"selected='selected'":""); ?>
				>
				<?php echo($row_ha["tenalbum"]); ?>
				</option>
				<?php 
				} 
				?>
        ...
        <tr>
          <td>Trạng thái:</td>
          <td><label>
            <input type="checkbox" name="trangthai" id="trangthai" <?php echo($row_edit_ha["trangthai"]=="1"?"checked='checked'":""); ?> />
          </label></td>
        </tr>
        <tr>
          <td>
	...
	<script type="text/javascript">
		...
		{
			document.getElementById( 'duongdananh' ).value = fileUrl ;
		}
          ...
          <td><label>
            <input type="submit" name="editha" id="editha" value="Lưu" />
            <input type="reset" name="reset" id="reset" value="Xóa" />
            <input type="hidden" name="editha" id="editha" />
          </label></td>
        </tr>
        <?php } ?>
      </table></form></td>
